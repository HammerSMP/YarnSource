...
public abstract class NumberRange<T extends Number> {
    ...
    protected static <T extends Number, R extends NumberRange<T>> R fromJson(@Nullable JsonElement jsonElement, R arg, BiFunction<JsonElement, String, T> biFunction, Factory<T, R> arg2) {
    ...
    protected static <T extends Number, R extends NumberRange<T>> R parse(StringReader stringReader, CommandFactory<T, R> arg, Function<String, T> function, Supplier<DynamicCommandExceptionType> supplier, Function<T, T> function2) throws CommandSyntaxException {
        if (!stringReader.canRead()) {
            throw EXCEPTION_EMPTY.createWithContext((ImmutableStringReader)stringReader);
        }
        ...
        try {
            ...
            if (stringReader.canRead(2) && stringReader.peek() == '.' && stringReader.peek(1) == '.') {
                stringReader.skip();
                stringReader.skip();
                Number number2 = (Number)NumberRange.map(NumberRange.fromStringReader(stringReader, function, supplier), function2);
            ...
            return arg.create(stringReader, number, number3);
        }
    ...
    extends NumberRange<Float> {
        ...
        public static FloatRange parse(StringReader stringReader, Function<Float, Float> function) throws CommandSyntaxException {
            return FloatRange.parse(stringReader, FloatRange::create, Float::parseFloat, ((BuiltInExceptionProvider)CommandSyntaxException.BUILT_IN_EXCEPTIONS)::readerInvalidFloat, function);
        }
    }
    ...
    extends NumberRange<Integer> {
        ...
        public static IntRange fromStringReader(StringReader stringReader, Function<Integer, Integer> function) throws CommandSyntaxException {
            return IntRange.parse(stringReader, IntRange::parse, Integer::parseInt, ((BuiltInExceptionProvider)CommandSyntaxException.BUILT_IN_EXCEPTIONS)::readerInvalidInt, function);
        }
    }
}
