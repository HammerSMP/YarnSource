...
extends DataFix {
    ...
    private static <T> Stream<Dynamic<T>> fixLoreTags(Stream<Dynamic<T>> stream) {
        return stream.map(dynamic -> (Dynamic)DataFixUtils.orElse((Optional)dynamic.asString().map(ItemLoreToTextFix::componentize).map(((Dynamic)dynamic)::createString).result(), (Object)dynamic));
    }
