...
extends AbstractUuidFix {
    ...
    protected TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("EntityUUIDFixes", this.getInputSchema().getType(this.typeReference), typed -> {
            ...
            for (String string2 : TAMEABLE_PETS) {
                typed = this.updateTyped((Typed<?>)typed, string2, EntityUuidFix::updateTameable);
            }
            for (String string3 : BREEDABLES) {
                typed = this.updateTyped((Typed<?>)typed, string3, EntityUuidFix::updateBreedable);
            }
            for (String string4 : LEASHABLES) {
                typed = this.updateTyped((Typed<?>)typed, string4, EntityUuidFix::updateLeashable);
            }
            for (String string5 : OTHER_LIVINGS) {
                typed = this.updateTyped((Typed<?>)typed, string5, EntityUuidFix::updateLiving);
            }
            for (String string6 : PROJECTILES) {
                typed = this.updateTyped((Typed<?>)typed, string6, EntityUuidFix::updateProjectile);
            }
    ...
    private static Dynamic<?> updateFox(Dynamic<?> dynamic) {
        Optional optional = dynamic.get("TrustedUUIDs").map(dynamic22 -> dynamic.createList(dynamic22.asStream().map(dynamic -> EntityUuidFix.createArrayFromCompoundUuid(dynamic).orElseGet(() -> {
    ...
    private static Dynamic<?> updateProjectile(Dynamic<?> dynamic) {
        return (Dynamic)DataFixUtils.orElse((Optional)dynamic.get("OwnerUUID").map(dynamic2 -> dynamic.remove("OwnerUUID").set("Owner", dynamic2)), dynamic);
    }
