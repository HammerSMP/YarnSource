...
extends Biome {
    public WarmOceanBiome() {
        super(new Biome.Settings().configureSurfaceBuilder(class_5471.FULL_SAND).precipitation(Biome.Precipitation.RAIN).category(Biome.Category.OCEAN).depth(-1.0f).scale(0.1f).temperature(0.5f).downfall(0.5f).effects(new BiomeEffects.Builder().waterColor(4445678).waterFogColor(270131).fogColor(12638463).moodSound(BiomeMoodSound.CAVE).build()).parent(null));
        ...
        this.addFeature(GenerationStep.Feature.VEGETAL_DECORATION, class_5464.WARM_OCEAN_VEGETATION);
        this.addFeature(GenerationStep.Feature.VEGETAL_DECORATION, class_5464.SEAGRASS_WARM);
        this.addFeature(GenerationStep.Feature.VEGETAL_DECORATION, class_5464.SEA_PICKLE);
