...
extends Biome {
    public CrimsonForestBiome() {
        super(new Biome.Settings().configureSurfaceBuilder(class_5471.CRIMSON_FOREST).precipitation(Biome.Precipitation.NONE).category(Biome.Category.NETHER).depth(0.1f).scale(0.2f).temperature(2.0f).downfall(0.0f).effects(new BiomeEffects.Builder().waterColor(4159204).waterFogColor(329011).fogColor(0x330303).particleConfig(new BiomeParticleConfig(ParticleTypes.CRIMSON_SPORE, 0.025f)).loopSound(SoundEvents.AMBIENT_CRIMSON_FOREST_LOOP).moodSound(new BiomeMoodSound(SoundEvents.AMBIENT_CRIMSON_FOREST_MOOD, 6000, 8, 2.0)).additionsSound(new BiomeAdditionsSound(SoundEvents.AMBIENT_CRIMSON_FOREST_ADDITIONS, 0.0111)).music(MusicType.method_27283(SoundEvents.MUSIC_NETHER_CRIMSON_FOREST)).build()).parent(null));
        ...
        this.addFeature(GenerationStep.Feature.VEGETAL_DECORATION, class_5464.WEEPING_VINES);
        this.addFeature(GenerationStep.Feature.VEGETAL_DECORATION, class_5464.CRIMSON_FUNGI);
        this.addFeature(GenerationStep.Feature.VEGETAL_DECORATION, class_5464.CRIMSON_FOREST_VEGETATION);
