...
extends CompositeEntityModel<T> {
    ...
    public void animateModel(T arg, float f, float g, float h) {
        for (int i = 1; i < 3; ++i) {
            this.field_3612[i].yaw = (((WitherEntity)arg).getHeadYaw(i - 1) - ((WitherEntity)arg).bodyYaw) * ((float)Math.PI / 180);
            this.field_3612[i].pitch = ((WitherEntity)arg).getHeadPitch(i - 1) * ((float)Math.PI / 180);
        }
    }
