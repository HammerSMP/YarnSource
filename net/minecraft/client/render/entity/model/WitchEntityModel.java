...
extends VillagerResemblingModel<T> {
    ...
    public void setAngles(T arg, float f, float g, float h, float i, float j) {
        ...
        float k = 0.01f * (float)(((Entity)arg).getEntityId() % 10);
        this.nose.pitch = MathHelper.sin((float)((Entity)arg).age * k) * 4.5f * ((float)Math.PI / 180);
        this.nose.yaw = 0.0f;
        this.nose.roll = MathHelper.cos((float)((Entity)arg).age * k) * 2.5f * ((float)Math.PI / 180);
        if (this.liftingNose) {
